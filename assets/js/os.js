...
// Function to click the radio button to select the algorithm from list of algorithms
...
// Function to add rows to table for the addition of processes 
function addRow(){
	let AT=document.getElementById("AT").value;
    let BT=document.getElementById("BT").value;
    if(AT=="" ||BT==""){
    	alert("Arrival or Burst time is not entered!");
    }
    else{
        if(Number(AT)<0 || Number (BT)<0){
            alert("Invalid Values of Arrival and Burst time!");
            return;
        }
...
//Function to create graph for the time taken by each process`
function create_Graph() {
    ...
    // Getting element by id my chart and creating a chart as mentioned in in the index.html file line no : 145
...
function calculate(){
    ...
    //     create_Graph();
    // }
    //________________________________End_______________________________________________
